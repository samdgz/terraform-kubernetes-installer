...
resource "oci_core_instance" "ServiceProxyInstanceAD1" {
  ...
  display_name        = "${(var.region == "us-ashburn-1") ? "ociash1-${var.regulatory_domain}-patronus-lbp-01" : "ociphx1-${var.regulatory_domain}-patronus-lbp-01"}"
...
resource "oci_core_instance" "ServiceProxyInstanceAD2" {
  ...
  compartment_id      = "${(var.multiple_compartments == "true")  ? var.bastion_compartment_ocid : var.compartment_ocid}"
  display_name        = "${(var.region == "us-ashburn-1") ? "ociash1-${var.regulatory_domain}-patronus-lbp-02" : "ociphx1-${var.regulatory_domain}-patronus-lbp-02"}"
  image               = "${lookup(data.oci_core_images.ServiceProxyImageOCID.images[0], "id")}"
  shape               = "${var.serviceproxy_instance_shape}"
...
resource "oci_core_instance" "ServiceProxyInstanceAD3" {
  ...
  compartment_id      = "${(var.multiple_compartments == "true")  ? var.bastion_compartment_ocid : var.compartment_ocid}"
  display_name        = "${(var.region == "us-ashburn-1") ? "ociash1-${var.regulatory_domain}-patronus-lbp-03" : "ociphx1-${var.regulatory_domain}-patronus-lbp-03"}"
  image               = "${lookup(data.oci_core_images.ServiceProxyImageOCID.images[0], "id")}"
  shape               = "${var.serviceproxy_instance_shape}"
